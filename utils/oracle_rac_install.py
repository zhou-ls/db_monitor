...
class OracleRacInstall():
    ...
    def upload_software(self, linux_params):
        ...
        LinuxBase(linux_params).sftp_upload_file(
            '{}compat-libstdc++-33-3.2.3-72.el7.x86_64.rpm'.format(self.local_path),
    ...
    def linux_config(self, node1, node2):
        ...
        ip_conf = "127.0.0.1   localhost localhost.localdomain localhost4 localhost4.localdomain4\n" \
                  ...
                  "{}  {}-cluster   {}-cluster-scan".format(node1['ip'], self.rac_info['hostname'] + node1['node_id'],
                                                            ...
                                                            node1['ip_vip'],
                                                            self.rac_info['hostname'] + node1['node_id'],
                                                            node2['ip_vip'],
                                                            self.rac_info['hostname'] + node2['node_id'],
                                                            node1['ip_priv'],
                                                            self.rac_info['hostname'] + node1['node_id'],
                                                            node2['ip_priv'],
                                                            self.rac_info['hostname'] + node2['node_id'],
    ...
    def grid_install(self, node):
        ...
        LinuxBase(linux_params).sftp_upload_file('{}LINUX.X64_193000_grid_home.zip'.format(self.local_path),
        ...
        self.log(
            'grid集群配置成功，请在1号节点{}上使用grid用户执行先决条件检查：$ORACLE_HOME/runcluvfy.sh stage -pre crsinst -responseFile /tmp/gridsetup.rsp'.format(
                node['ip']))

        self.log(
            '在先决条件检查完成后，请在1号节点{}上执行静默安装：/u01/app/19.0.0/grid/gridSetup.sh -silent -ignorePrereqFailure -responseFile /tmp/gridsetup.rsp'
    ...
    def oracle_install(self, node):
        ...
        LinuxBase(linux_params).sftp_upload_file('{}LINUX.X64_193000_db_home.zip'.format(self.local_path),
    ...
    def oracle_dbca(self, node):
        ...
        self.log(
            'dbca配置成功，请在1号节点{}上使用oracle用户执行dbca建库脚本：/u01/app/oracle/product/19.0.0/dbhome_1/bin/dbca -silent -createDatabase -ignorePrereqFailure '
