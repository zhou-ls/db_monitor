...
def check_oracle(tags, oracle_params):
    ...
    if db_conn and db_conn_cdb:
        ...
        insert_data_values = {**locals(), **oracle_osstat, **oracle_wait, **oracle_load, **oracle_sess, **oracle_stat,
                              **oracle_mem}
        ...
        checklog.logger.info(
            '{}：获取Oracle数据库监控数据(数据库名：{} 数据库角色：{} 数据库状态：{})'.format(tags, dbname, database_role, open_mode))
