...
class ApiOracleStatHis(generics.ListCreateAPIView):
    def get_queryset(self):
        ...
        if start_time and end_time:
            ...
            end_time = today()
            start_time = last_day()
        return OracleStatHis.objects.filter(tags=tags, check_time__gte=start_time, check_time__lte=end_time).order_by(
            'check_time')
...
class ApiOracleTableSpaceHis(generics.ListCreateAPIView):
    def get_queryset(self):
        ...
        if start_time and end_time:
            ...
            end_time = today()
            start_time = last_day()
        return OracleTableSpaceHis.objects.filter(tags=tags, tablespace_name=tablespace_name,
                                                  check_time__gte=start_time, check_time__lte=end_time).order_by(
            'check_time')
...
def ApiOracleTableSpaceDayUsed(request):
    ...
    sql = "select to_char(trunc(to_date(rtime, 'mm/dd/yyyy hh24:mi:ss'), 'dd'),'yyyy-mm-dd') m_date,b.name," \
          ...
          "group by to_char(trunc(to_date(rtime, 'mm/dd/yyyy hh24:mi:ss'), 'dd'),'yyyy-mm-dd'),b.name".format(
        tablespace_name)
...
class ApiOracleTempTableSpaceHis(generics.ListCreateAPIView):
    def get_queryset(self):
        ...
        if start_time and end_time:
            ...
            end_time = today()
            start_time = last_day()
        return OracleTempTableSpaceHis.objects.filter(tags=tags, temptablespace_name=temptablespace_name,
                                                      check_time__gte=start_time, check_time__lte=end_time).order_by(
            'check_time')
...
def ApiOracleTempTableSpaceSessionUsed(request):
    ...
    sql = "SELECT S.sid ,S.username, S.osuser, P.spid, S.module,S.program,SUM(T.blocks) * TBS.block_size / 1024/1024  mb_used," \
          ...
          "GROUP BY S.sid,S.serial#,S.username,S.osuser,P.spid,S.module,S.program,TBS.block_size,T.tablespace".format(
        temptablespace_name)
...
class ApiOracleUndoTableSpaceHis(generics.ListCreateAPIView):
    def get_queryset(self):
        ...
        if start_time and end_time:
            ...
            end_time = today()
            start_time = last_day()
        return OracleUndoTableSpaceHis.objects.filter(tags=tags, undotablespace_name=undotablespace_name,
                                                      check_time__gte=start_time, check_time__lte=end_time).order_by(
            'check_time')
...
def ApiOracleUndoTableSpaceSessionUsed(request):
    ...
    sql = "SELECT r.name rbs,nvl(s.username, 'None') oracle_user,s.osuser client_user,p.username unix_user,s.program,s.sid," \
          ...
          " WHERE s.taddr = t.addr AND s.paddr = p.addr AND r.usn = t.xidusn(+) AND x.name = 'db_block_size' ".format(
        undotablespace_name)
    sessionused_list = OracleBase(oracle_params).django_query(sql)
...
def ApiOracleUserRole(request):
    ...
    oracle_params = get_oracle_params(tags)
    sql = "select grantee,granted_role, admin_option,default_role from dba_role_privs where grantee = '{}' ".format(
        user)
